 ...
 */
if ( ! class_exists( 'Custom_WP_Posts_List_Table' ) ) {
		...
		public function column_title( $post ) {
			...
			if ( $can_edit_post && 'trash' !== $post->post_status ) {
				...
				if ( $lock_holder ) {
					...
					$locked_text = esc_html( sprintf( __( '%s is currently editing', 'default' ), $lock_holder->display_name ) );
				} else {
					$locked_avatar = '';
					$locked_text   = '';
				}
			...
			if ( $can_edit_post && 'trash' !== $post->post_status ) {
				printf(
					...
					esc_attr( sprintf( __( '&#8220;%s&#8221; (Edit)', 'default' ), $title ) ),
					$pad,
					$title
				);
			} else {
