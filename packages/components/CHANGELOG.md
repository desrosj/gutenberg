...
-   `Button`: Migrate hardcoded Sass color, spacing, and typography values to WPDS design tokens, aligning brand, pressed, and destructive variant colors with the interactive tokens used by `@wordpress/ui` Button while preserving admin theme color support via token fallbacks.
