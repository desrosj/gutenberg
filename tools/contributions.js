...
coAuthorData.reporters = new Set();
coAuthorData.linkedIssueCommenters = new Set();
...
for (const linkedIssue of contributorData.repository.pullRequest.closingIssuesReferences.nodes){
	if (!coAuthorData.committers.has(linkedIssue.author.login) && !coAuthorData.reviewers.has(linkedIssue.author.login) && !coAuthorData.commenters.has(linkedIssue.author.login)) {
	...
	for (const issueComment of linkedIssue.comments.nodes){
		...
		if (!coAuthorData.committers.has(issueComment.author.login) && !coAuthorData.reviewers.has(issueComment.author.login) && !coAuthorData.commenters.has(issueComment.author.login) && !coAuthorData.reporters.has(issueComment.author.login)) {
